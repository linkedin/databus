...
public class TestDbusEventBuffer {
  ...
  static
  {
    ...
    ScnIndex.LOG.setLevel(Level.INFO);
  }
...
public static class DummyDbusEvent extends DbusEvent
{
    ...
    @Test
    public void testReadEventsMidError() throws Exception
    {
    	final int numSrcEvents = 20;
        //setup buffer
        final Logger log = Logger.getLogger("TestDbusEventBuffer.testReadEventsMidError");
        //log.setLevel(Level.DEBUG);
        //DbusEventBuffer.LOG.setLevel(Level.INFO);
        final DbusEventBuffer.StaticConfig srcBufferCfg = getConfig(100000, 100000, 128, 128,
                                                              AllocationPolicy.HEAP_MEMORY,
                                                              QueuePolicy.BLOCK_ON_WRITE,
                                                              AssertLevel.ALL);
        final DbusEventBuffer srcBuf = new DbusEventBuffer(srcBufferCfg);
        srcBuf.start(0);
        final DbusEventsStatisticsCollector srcBufStats =
            new DbusEventsStatisticsCollector(1, "src", true, false, null);
        final DbusEventGenerator evGen = new DbusEventGenerator(10);

        final DbusEventBuffer.StaticConfig destBufferCfg1 =
            getConfig(100000, 100000, 128,
            		100000,
                      AllocationPolicy.HEAP_MEMORY,
                      QueuePolicy.BLOCK_ON_WRITE, AssertLevel.NONE);
        final DbusEventBuffer destBuf = new DbusEventBuffer(destBufferCfg1);
        destBuf.setDropOldEvents(true);
        destBuf.start(0);

        final DbusEventsStatisticsCollector destBufStats1 =
            new DbusEventsStatisticsCollector(1, "dest1", true, false, null);


        final ByteArrayOutputStream srcByteStr = new ByteArrayOutputStream();

        //generate events
        final Vector<DbusEvent> srcEvents =
            generateEventsForTest(100000, srcBuf, srcBufStats, srcByteStr, evGen,
            					  numSrcEvents,
                                  2);

        //save the failover response for later
        final ByteArrayOutputStream saveSrcBytesStr = new ByteArrayOutputStream();
        saveSrcBytesStr.write(srcByteStr.toByteArray());

        //now add an invalid byte sequence
        final String badMsg = "DEADBEEF";
        srcByteStr.write(badMsg.getBytes());

        final ReadableByteChannel readChannel1 =
                Channels.newChannel(new ByteArrayInputStream(srcByteStr.toByteArray()));
        final AtomicInteger eventsRead1 = new AtomicInteger(-1);
        final AtomicBoolean hasError1 = new AtomicBoolean(false);

        log.info("run the convumer 1 read in a separate thread in case it hangs");
        final Thread readThread1 = new Thread(new Runnable()
        {
          @Override
          public void run()
          {
            try
            {
              eventsRead1.set(destBuf.readEvents(readChannel1, destBufStats1));
            }
        ...
        readThread1.join(1000);

        log.info("consumer 1 smoke tests");
        Assert.assertTrue(!readThread1.isAlive());
        //there should be a failure
        Assert.assertEquals(true, hasError1.get());

        log.info("verify that consumer did not get any events");
        Vector<DbusEvent> destEvents1 = new Vector<DbusEvent>(numSrcEvents);
        DbusEventBufferConsumer consumer =
            new DbusEventBufferConsumer(destBuf, numSrcEvents, 0, destEvents1);
        boolean consumerDone = consumer.runWithTimeout(1000);
        Assert.assertEquals(false, consumerDone);
        Assert.assertEquals(0, destEvents1.size());
        readChannel1.close();
        destBufStats1.reset();

        log.info("now write the events again without the error simulating a failover to a different relay");
        final ReadableByteChannel readChannel2 =
                Channels.newChannel(new ByteArrayInputStream(saveSrcBytesStr.toByteArray()));
        final AtomicInteger eventsRead2 = new AtomicInteger(-1);
        final AtomicBoolean hasError2 = new AtomicBoolean(false);
        final Thread readThread2 = new Thread(new Runnable()
        {
          @Override
          public void run()
          {
            try
            {
              eventsRead2.set(destBuf.readEvents(readChannel2, destBufStats1));
            }
            catch (InvalidEventException e)
            {
              log.error("readEvents error: " + e.getMessage(), e);
        ...
        readThread2.setDaemon(true);
        readThread2.start();

        log.info("wait for read 2 to finish");
        readThread2.join(10000000);
        readChannel2.close();

        log.info("process events in consumer 2");
        final DbusEventBufferConsumer consumer2 =
                new DbusEventBufferConsumer(destBuf, numSrcEvents, 0, destEvents1);
        final boolean consumer2Done = consumer2.runWithTimeout(1000);
        Assert.assertEquals(true, consumer2Done);

        log.info("verify events in consumer 2");
        checkEvents(srcEvents, destEvents1, numSrcEvents);

        srcByteStr.close();
        saveSrcBytesStr.close();
    }

    private Vector<DbusEvent> generateEventsForTest(final int srcBufferSize,
                                                    final DbusEventBuffer srcBuf,
                                                    final DbusEventsStatisticsCollector srcBufStats,
                                                    final ByteArrayOutputStream srcByteStr,
                                                    final DbusEventGenerator evGen,
                                                    final int numSrcEvents,
                                                    final int maxWindowSize) throws ScnNotFoundException,
        OffsetNotFoundException
    {
      final Vector<DbusEvent> srcEvents = new Vector<DbusEvent>(numSrcEvents);
      Assert.assertTrue(evGen.generateEvents(numSrcEvents, maxWindowSize, 200, 100, srcEvents) > 0);

      //generate binary representation
      final DbusEventAppender app = new DbusEventAppender(srcEvents, srcBuf, srcBufStats);
      app.run();
      Assert.assertEquals(numSrcEvents, srcBufStats.getTotalStats().getNumDataEvents());
      final WritableByteChannel srcChannel = Channels.newChannel(srcByteStr);
      Checkpoint cp = new Checkpoint();
      cp.setFlexible();
      srcBufStats.reset();
      srcBuf.streamEvents(cp, false, srcBufferSize, srcChannel, Encoding.BINARY,
                          new AllowAllDbusFilter(), srcBufStats);
      Assert.assertEquals(numSrcEvents, srcBufStats.getTotalStats().getNumDataEvents());
      return srcEvents;
    }
    ...
    private void readEventsBlocking (boolean invokeStartOnBuffer)
      ...
      //set staging buffer size = shared buffer size ; shared buffer is still lower  than total capacity ;
        ...
        for (Iterator<EventBufferTestInput> it=tests.iterator(); it.hasNext(); ) {
          ...
          System.out.printf("TestId=%d Test=%s  result=%b size of dst events=%d \n",testId, testInput.toString(),result,dstTestEvents.size());
          assertTrue(result);
    	...
    	throws Exception
    {
		...
		long seq = events.get(0).sequence();
    	// Add events to the EventBuffer
    	DbusEventAppender appender = new DbusEventAppender(events,dbusBuf,null);
    	appender.run();
...
*/
  ...
  public void testResetBuffer() throws Exception
  {
	...
	protected void checkEvents(Vector<DbusEvent> srcTestEvents, Vector<DbusEvent> dstTestEvents,int numEvents) {
	  //check data!
