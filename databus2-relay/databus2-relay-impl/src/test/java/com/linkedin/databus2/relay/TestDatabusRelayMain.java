...
public class TestDatabusRelayMain
{
			...
			int maxBufferSize, PhysicalSourceConfig[] sourceConfigs)
	{
		...
		{
			System.err.println("Databus Exception " + e);
		}
		return null;
	}
	...
	protected void resetSCN(DatabusRelayMain relay)
	{
		for (PhysicalSourceStaticConfig pConfig : relay.getPhysicalSources())
		{
			...
			if (scnWriters != null)
			{
				...
				{
					System.err.println("Warn: cannot clear scn" + e);
				}
			}
			else
			{
				System.err.println("Could not find scn writer for " + pConfig.getPhysicalPartition());
			}
		}
	}

	@Test
	public void testRelayEventGenerator()
	{
		try
		{
			String[][] srcNames =
			...
			RelayRunner r1 = new RelayRunner(relay1);
			...
			DatabusSourcesConnection clientConn = RelayEventProducer
					.createDatabusSourcesConnection("testProducer", serverName,
							...
							1, true);
			ClientRunner cr = new ClientRunner(clientConn);
			cr.start();
			...
			do
			{
				System.out.println("numDataEvents=" + stats.getNumDataEvents()
			...
			r1.pause();
			System.out.println("Sending pause to relay!");
			System.out.println("numDataEvents=" + stats.getNumDataEvents()
					+ " numWindows=" + stats.getNumSysEvents() + " size="
					+ stats.getSizeDataEvents());
			Thread.sleep(5000);
			...
			System.out.println("Client stats=" + countingConsumer);
			System.out.println("Event windows generated="
					+ stats.getNumSysEvents());

			boolean stopped = r1.shutdown(2000);
			Assert.assertTrue(stopped);
			cr.shutdown();
			System.out.println("Client cr stopped");
			Assert.assertTrue(countingConsumer.getNumDataEvents() == stats
					.getNumDataEvents());
		}
		catch (Exception e)
		{
	 ...
	 */
	public void testRelayChainingBasic()
	{
		try
		{
			String[][] srcNames =
			...
			DatabusRelayMain relay2 = createDatabusRelay(1002,
					chainedRelayPort, 1 * 1024 * 1024, chainedSrcConfigs);
			Assert.assertTrue(null != relay2);
			RelayRunner r2 = new RelayRunner(relay2);
			...
			System.out.println("numDataEvents=" + stats.getNumDataEvents()
					+ " numWindows=" + stats.getNumSysEvents() + " size="
					+ stats.getSizeDataEvents());
			System.out.println("numDataEvents2=" + stats2.getNumDataEvents()
	...
	public void testRelayChainingConnectFailure()
	{
		try
		{
			...
			while (countingConsumer.getNumWindows() < stats.getNumSysEvents())
			{
				Thread.sleep(500);
				// System.out.println("Client stats=" + countingConsumer);
				// System.err.println("numDataEvents=" +
	...
	public void testRelayChainingPartialSubscribeRelay()
	{
		try
		{
			...
			System.out.println("Client stats=" + countingConsumer);
			System.out.println("Chained stats="
					+ statsChained.getNumSysEvents());
			System.out.println("Event windows generated="
					+ stats.getNumSysEvents());
			System.err.println("numDataEvents=" + stats.getNumDataEvents()
					+ " numWindows=" + stats.getNumSysEvents() + " size="
					+ stats.getSizeDataEvents());
	 ...
	 */
	public void testRelayChainingRestartSCNOffset()
	{
		try
		{
			String[][] srcNames =
			...
			chainedSrcConfigs[0].setRestartScnOffset(stats.getMaxScn() - stats.getPrevScn());
			DatabusRelayMain relay3 = createDatabusRelay(1002,
					chainedRelayPort, 1 * 1024 * 1024, chainedSrcConfigs);
			RelayRunner r3 = new RelayRunner(relay3);
			r3.start();
			
			Thread.sleep(5*1000);
			
			DbusEventsTotalStats stats3 = relay3
					.getInboundEventStatisticsCollector().getTotalStats();
			Assert.assertTrue(stats3.getNumDataEvents() > 0);
			
			System.out.println("Stats3= numDataEvents=" + stats3.getNumDataEvents()
					+ " numWindows=" + stats3.getNumSysEvents() + " size="
					+ stats3.getSizeDataEvents());
	 ...
	 */
	public void testRelayChainingSCNRegress()
	{
		try
		{
			String[][] srcNames =
			...
			DatabusRelayMain relay1 = createDatabusRelay(1001, relayPort,
					10 * 1024 * 1024, srcConfigs);
			DatabusRelayMain relay3 = createDatabusRelay(1001, relayPort,
					10 * 1024 * 1024, srcConfigs);
			Assert.assertTrue(null != relay1);
			Assert.assertTrue(null != relay3);
			RelayRunner r1 = new RelayRunner(relay1);
			DbusEventsTotalStats stats = relay1
					.getInboundEventStatisticsCollector().getTotalStats();
			DbusEventsTotalStats stats3 = relay3
			...
			System.out.println("numDataEvents1=" + firstGenDataEvents
			...
			System.out.println("numDataEvents2=" + firstGenChainWindows
			...
			System.out.println("numDataEvents3=" + stats3.getNumDataEvents()
					+ " numWindows3=" + stats3.getNumSysEvents() + " minScn="
					+ stats3.getMinScn() +  " maxScn= " + stats3.getMaxScn());
			
			System.out.println("numDataEvents22=" + stats2.getNumDataEvents()
					+ " numWindows22=" + stats2.getNumSysEvents() + " minScn2="
					+ stats2.getMinScn() + " maxScn2=" + stats2.getMaxScn());

			System.out.println("consumer=" + countingConsumer);
			
			//compare chained relays with 2 gens of tier 0 relays
			Assert.assertTrue(stats2.getMinScn() ==firstGenMinScn) ;
			Assert.assertTrue(stats2.getMaxScn() == stats3.getMaxScn());
			...
			Assert.assertTrue(stats2.getNumSysEvents() == (firstGenChainWindows-1) + stats3.getNumSysEvents());
			Assert.assertTrue(stats2.getNumDataEvents() > stats3.getNumDataEvents());
			
			//compare source to final client
			Assert.assertTrue(countingConsumer.getNumSources() == 2);
			Assert.assertTrue(stats2.getNumSysEvents() == countingConsumer
					.getNumWindows());

			boolean sorted= true;
			long prev = -1;
			System.out.print(" scn seq on consumer=");
	...
	static public class RelayRunner extends Thread
	{
		...
		public boolean shutdown(int timeoutMs)
		{
			...
			{
				System.err.println("Not shutdown in " + timeoutMs + " ms");
				return false;
			}
			catch (InterruptedException e)
			{
				System.out.println("Interrupted before " + timeoutMs + " ms");
				return true;
			}
			return true;
		}
